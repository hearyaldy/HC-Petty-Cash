...
class ManagedFirestoreService {
  ...
  // Shared streams with proper caching to avoid multiple listeners
  ...
  Stream<List<PettyCashReport>> reportsStream() {
    ...
    if (!_streamControllers.containsKey(cacheKey) ||
      ...
      final controller = BehaviorSubject<List<PettyCashReport>>();
      ...
      final subscription = _reportsCollection
          ...
          .listen(
            controller.add,
            onError: (error) {
              _handleStreamError(cacheKey, error);
  ...
  /// Reset a specific stream by cache key - useful when Firestore web gets into bad state
  void resetStream(String cacheKey) {
    if (_subscriptions.containsKey(cacheKey)) {
  ...
  /// Reset all streams - call this when navigating away or on Firestore errors
  void resetAllStreams() {
    ...
    _streamControllers.clear();
    AppLogger.info('All Firestore streams have been reset');
  }

  /// Check if a stream error is a Firestore web internal error
  ...
  // Individual operations remain the same
  ...
  String generateTravelingReportNumber() {
    ...
    return 'TR-$dateStr-${uuid.substring(0, 3).toUpperCase()}';
  }
  ...
  Future<IncomeReport> createIncomeReport({
    ...
    try {
      ...
      await _incomeReportsCollection.doc(reportId).set(report.toFirestore());
      return report;
    } catch (e) {
  ...
  Future<IncomeEntry> addIncomeEntry({
    ...
    try {
      ...
      await _incomeEntriesCollection.doc(entryId).set(entry.toFirestore());
  ...
  String generatePurchaseRequisitionNumber() {
    final now = DateTime.now();
    final dateStr = DateFormat('yyyyMMdd').format(now);
    final uuid = 'UUID-V4-SAMPLE'; // Replace with actual UUID
    return 'PR-$dateStr-${uuid.substring(0, 3).toUpperCase()}';
  }
