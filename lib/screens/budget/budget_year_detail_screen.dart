...
class BudgetYearDetailScreen extends StatefulWidget {
  ...
  @override
  State<BudgetYearDetailScreen> createState() =>
      _BudgetYearDetailScreenState();
}
...
class _BudgetYearDetailScreenState extends State<BudgetYearDetailScreen>
  ...
  Future<void> _approve() async {
    ...
    if (confirm == true && mounted) {
      await context
          .read<BudgetProvider>()
          .approveYear(auth.currentUser!.id);
      if (mounted) {
                    ...
                    Expanded(
                      child: TabBarView(
                        ...
                        children: [
                          ...
                          _buildTabContent(provider, 'appropriation'),
                        ],
                      ),
                    ),
                  ],
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final pills = Wrap(
            ...
            runSpacing: 6,
            children: [
              _summaryPill('Income', p.totalIncomeBudget, Colors.greenAccent),
              _summaryPill('Expense', p.totalExpenseBudget, Colors.orangeAccent),
              _summaryPill('Net Budget', netBudget,
                  netBudget >= 0 ? Colors.greenAccent : Colors.redAccent),
              if (p.totalIncomeActual > 0 || p.totalExpenseActual > 0)
                _summaryPill('Net Actual', netActual,
    ...
    return Container(
      ...
      child: TabBar(
        ...
        tabs: const [
          ...
          Tab(icon: Icon(Icons.swap_horiz, size: 16), text: 'Appropriations'),
        ],
      ),
    );
  }
  ...
  Widget _buildSection(BudgetSection section, BudgetProvider provider) {
    ...
    final actualTotal =
        section.items.fold(0.0, (s, i) => s + (provider.actuals[i.id] ?? 0));
      ...
      child: Column(
        children: [
              ...
              child: Row(
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      Text('฿${_fmt.format(section.budgetTotal)}',
                          style: const TextStyle(
          ...
          if (!isCollapsed) ...[
            ...
            ...section.items
                .map((item) => _buildLineItemRow(item, provider)),
          ],
        ],
      ),
    );
  ...
  Widget _buildTypeTotal(
      List<BudgetSection> sections, String type, BudgetProvider provider) {
    final cs = Theme.of(context).colorScheme;
    final budgetTotal =
        sections.fold(0.0, (s, sec) => s + sec.budgetTotal);
    final actualTotal = sections.fold(
        0.0,
        (s, sec) =>
            s +
            sec.items
                .fold(0.0, (ss, i) => ss + (provider.actuals[i.id] ?? 0)));
      ...
      child: Row(
        children: [
          ...
          Column(
            crossAxisAlignment: CrossAxisAlignment.end,
            children: [
              Text('฿${_fmt.format(budgetTotal)}',
                  style: TextStyle(
                      fontSize: 14,
                      fontWeight: FontWeight.bold,
                      color: cs.onPrimaryContainer)),
              if (actualTotal > 0)
                Text('Actual: ฿${_fmt.format(actualTotal)}',
                    style: TextStyle(
                        fontSize: 11,
                        color:
